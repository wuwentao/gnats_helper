...
getVersion(
	function(n){
		if(window.localStorage.cv!=n)
			...
			var t=window.webkitNotifications.createNotification("libs/icons/icon_48.png","Gnats Helper has been updated","Your Gnats Helper has been update to "+n+" :-)");
	...
	if(!n)
		{
		var t={urls:["*://*.juniper.net/*"],types:["main_frame","sub_frame","stylesheet","script","image","object","xmlhttprequest","other"]};
	...
	window.appId=="cmimcpeankkllapdbpfnflfamdehgngk"?(i=["bpbefagpafkfgoihbmcgeileodldkpnf","idjgmabfihmhmojipdkcackbihbdceno","iojocdfolilckogkigiahnjfmpjinobl"],chrome.management.getAll(function(r)
	{
	for(var f,u=0;u<r.length;u++)f=r[u],n=n||f.enabled&&$.inArray(f.id,i)!=-1;
	if(n){
		console.log("These duplicate gnats helper will be disable");
		return
		}
	t()
	})):t();
	window.chrome.extension.onRequest.addListener(function(t,i,r){var f=t.function,e,u,o,s;
	if(f=="notify"&&(e=window.webkitNotifications.createNotification("libs/icons/icon_48.png",t.title||"Gnats Helper",t.message),window.setTimeout(function(){e.cancel()},t.timeout||5e3),e.show()),f=="getAppId")
